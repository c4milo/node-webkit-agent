 ...
 * It's the controller for Webkit DevTools frontend and uses the debugger client to
 * forward commands
 *
...
(function(){
    ...
    this.setBreakpointByUrl = function(params, sendResult) {
        this.client.setBreakpoint(params, function(result) {
            sendResult(result);
        });
    };

    this.removeBreakpoint = function(params, sendResult) {
        this.client.removeBreakpoint(params, function(result) {
            sendResult(result);
        });
    };

    this.getScriptSource = function(params, sendResult) {
        this.client.getScriptSource(params, function(result) {
            sendResult(result);
        });
    };
